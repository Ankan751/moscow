...
import { X, Upload, Home, MapPin, Phone, DollarSign, BedDouble, Bath, Maximize, Link as LinkIcon, CheckSquare, Square, Plus, ArrowLeft, Loader2, Instagram, Youtube, Building, Sparkles } from 'lucide-react';
            ...
            <div className="space-y-4">
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <label htmlFor="description" className={cn(labelClass, "mb-0")}>Description</label>
                  <button
                    type="button"
                    onClick={async () => {
                      if (!formData.description || formData.description.length < 10) {
                        toast.error('Description is too short to enhance');
                        return;
                      }
                      setEnhancing(true);
                      const t = toast.loading('AI is enhancing your description...');
                      try {
                        const res = await apiClient.post('/api/admin/enhance-description', {
                          description: formData.description
                        });
                        if (res.data.success) {
                          setFormData(prev => ({ ...prev, description: res.data.enhanced }));
                          toast.success('Description enhanced!', { id: t });
                        } else {
                          toast.error(res.data.message || 'Enhancement failed', { id: t });
                        }
                      } catch (err) {
                        console.error(err);
                        toast.error('AI Service unavailable', { id: t });
                      } finally {
                        setEnhancing(false);
                      }
                    }}
                    disabled={enhancing || !formData.description}
                    className="flex items-center gap-1.5 px-3 py-1.5 bg-amber-50 text-amber-600 border border-amber-100 rounded-lg text-[10px] font-black uppercase tracking-wider hover:bg-amber-100 transition-all disabled:opacity-50"
                  >
                    {enhancing ? (
