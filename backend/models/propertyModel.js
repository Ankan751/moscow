...
// Pre-save hook to sync bhk and beds for compatibility
propertySchema.pre('save', function(next) {
  if (this.bhk && !this.beds) this.beds = this.bhk;
  if (this.beds && !this.bhk) this.bhk = this.beds;
  next();
});
...
propertySchema.index({ location: "text", title: "text", city: "text" }); // Text search (Original)
propertySchema.index({ price: 1, beds: 1, type: 1, location: 1 }); // Property filters (Original)
