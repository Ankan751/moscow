...
import { enhanceDescription } from '../services/grokService.js';
...
/** POST /api/admin/enhance-description — AI-driven description formalizer */
export const enhanceDescriptionController = async (req, res) => {
  try {
    const { description } = req.body;
    if (!description || description.trim().length < 10) {
      return res.status(400).json({ success: false, message: 'Description too short to enhance' });
    }
 ...
 * Get all properties regardless of status for admin management.
 */
export const getAllPropertiesForAdmin = async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      property: properties, // Use 'property' to match the frontend expectation in List.jsx
    });
  } catch (error) {
