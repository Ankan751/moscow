            ...
            result = await coalesce(cacheKey, async () => {
                ...
                try {
                    ...
                    analysis = {
                        ...
                        overview: propertiesData.properties.slice(0, 6).map(p => ({
                ...
                // Step 3: Sort properties by AI match score with price as tie-breaker
                if (analysis.overview && Array.isArray(analysis.overview)) {
                    // Create a map for quick lookup
                    const scoreMap = new Map();
                    analysis.overview.forEach(item => {
                        scoreMap.set(item.name.toLowerCase().trim(), item.match_score || 0);
                    });

                    // Update match_score on properties and sort
                    propertiesData.properties.forEach(p => {
                        p.match_score = scoreMap.get((p.building_name || '').toLowerCase().trim()) || 0;
                    });

                    const { parsePriceToCrores } = await import('../services/firecrawlService.js');

                    propertiesData.properties.sort((a, b) => {
