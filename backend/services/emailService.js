...
class EmailService {
  constructor() {
    this.fromAddress = process.env.EMAIL_USER || 'noreply@Hanumant Properties.com';
  }
  ...
  async sendWelcomeEmail(userEmail, userName) {
    const subject = 'Welcome to Hanumant Properties - Your Account is Ready!';
  ...
  async sendEmailVerification(userEmail, userName, verificationUrl) {
    const subject = 'Verify Your Hanumant Properties Email Address';
  ...
  async sendPasswordResetEmail(userEmail, resetUrl) {
    const subject = 'Reset Your Hanumant Properties Password';
  ...
  async sendNewsletterWelcome(userEmail) {
    const subject = 'Welcome to Hanumant Properties Newsletter 📧';
  ...
  async sendAppointmentNotificationToAdmin(appointment, userDetails) {
    ...
    const adminEmail = process.env.ADMIN_EMAIL || 'admin@Hanumant Properties.com';
  ...
  async sendUserSuspended(userEmail, userName, days, reason, suspendedUntil) {
    const subject = `Hanumant Properties Account Suspended - ${days} Day${days > 1 ? 's' : ''}`;
  ...
  async sendUserBanned(userEmail, userName, reason) {
    const subject = 'Hanumant Properties Account Permanently Banned';
  ...
  async sendUserReactivated(userEmail, userName) {
    const subject = 'Hanumant Properties Account Reactivated - Welcome Back!';
