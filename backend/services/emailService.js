...
class EmailService {
  constructor() {
    this.fromAddress = process.env.EMAIL_USER || 'noreply@Get A Dream Home.com';
  }
  ...
  async sendWelcomeEmail(userEmail, userName) {
    const subject = 'Welcome to Get A Dream Home - Your Account is Ready!';
  ...
  async sendEmailVerification(userEmail, userName, verificationUrl) {
    const subject = 'Verify Your Get A Dream Home Email Address';
  ...
  async sendPasswordResetEmail(userEmail, resetUrl) {
    const subject = 'Reset Your Get A Dream Home Password';
  ...
  async sendNewsletterWelcome(userEmail) {
    const subject = 'Welcome to Get A Dream Home Newsletter 📧';
  ...
  async sendAppointmentNotificationToAdmin(appointment, userDetails) {
    ...
    const adminEmail = process.env.ADMIN_EMAIL || 'admin@Get A Dream Home.com';
  ...
  async sendUserSuspended(userEmail, userName, days, reason, suspendedUntil) {
    const subject = `Get A Dream Home Account Suspended - ${days} Day${days > 1 ? 's' : ''}`;
  ...
  async sendUserBanned(userEmail, userName, reason) {
    const subject = 'Get A Dream Home Account Permanently Banned';
  ...
  async sendUserReactivated(userEmail, userName) {
    const subject = 'Get A Dream Home Account Reactivated - Welcome Back!';
