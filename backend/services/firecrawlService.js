 ...
 */
export function parsePriceToCrores(priceStr) {
 ...
 */
function filterValidProperties(properties, minPrice, maxPrice, isDefaultBudget = false) {
    const max = parseFloat(maxPrice) || 0;
    let min = parseFloat(minPrice) || 0;

    // Smart minimum price calculation:
    // Only applied if user provided an explicit budget (not default)
...
class FirecrawlService {
    ...
    async findProperties({
        ...
        try {
            ...
            const filtered = filterValidProperties(rawProperties, minPrice, maxPrice, isDefaultBudget);
