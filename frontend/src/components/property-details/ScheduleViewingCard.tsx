...
const ScheduleViewingCard: React.FC<ScheduleViewingCardProps> = ({ property }) => {
  ...
  if (success) {
    return (
      <div className="bg-[#E8F7ED] border border-[#22C55E]/20 rounded-2xl p-8 shadow-lg sticky top-8 text-center">
  ...
  return (
    <div className="bg-[#FBF4E4] border border-[#C5A059]/20 rounded-2xl p-8 shadow-lg sticky top-8">
      {/* Header */}
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <input
            ...
            className="w-full bg-white border border-[#E6E0DA] rounded-lg px-4 py-3 font-red-hat text-sm text-[#1C1B1A] placeholder:text-[#94A3B8] focus:outline-none focus:border-[#C5A059] transition-colors"
            required
          />
        </div>
        ...
        {/* Description */}
        <div>
          <label className="flex items-center gap-1.5 font-red-hat text-xs text-[#5A5856] uppercase tracking-wider mb-2 font-bold">
            Your Requirements
            <span className="px-1.5 py-0.5 bg-[#C5A059]/10 text-[#C5A059] rounded text-[10px]">AI</span>
          </label>
          <textarea
            name="description"
            ...
            rows={3}
            className="w-full bg-white border border-[#E6E0DA] rounded-lg px-4 py-3 font-red-hat text-sm text-[#1C1B1A] placeholder:text-[#94A3B8] focus:outline-none focus:border-[#C5A059] transition-colors resize-none"
          ></textarea>
        </div>

        {/* Error Message */}
        {error && (
          <p className="text-center font-red-hat text-xs text-red-500 mt-2">{error}</p>
        )}
        ...
        {/* Info Text */}
        <p className="text-center font-red-hat font-extralight text-xs text-[#94A3B8] mt-4">
